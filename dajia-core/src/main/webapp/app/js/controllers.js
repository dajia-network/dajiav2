angular
		...
		.controller(
				'ProdCtrl',
				function($scope, $http, $cookies, $ionicLoading, $window, AuthService, $timeout, $ionicModal,
						CouponService) {
					console.log('产品列表...');
					$scope.products = [];
					$scope.countDowns = [];
					...
					CouponService.canRequestCoupon(4, function(data) {
						console.log(data);
				...
				function($scope, $rootScope, $stateParams, $http, $cookies, $window, $timeout, $ionicSlideBoxDelegate,
					...
					$scope.add2Cart = function() {
						if ($cookies.get('dajia_user_id') == null) {
							...
							$http.get('/user/cart/add/' + $stateParams.pid).success(
									function(data, status, headers, config) {
								...
								if (null != $scope.loginuser.userContacts) {
									loginuser.userContacts.forEach(function(c) {
										var contact = {
											...
											summary : c.contactName + ' ' + c.contactMobile + ' '
													+ c.province.locationValue + ' ' + c.city.locationValue + ' '
													+ c.district.locationValue + ' ' + c.address1
										};
						...
						$http.post('/user/submitOrder', $scope.order).success(function(data, status, headers, config) {
							...
							if (null == charge || charge.length == 0) {
								...
								pingpp.createPayment(charge, function(result, error) {
									if (result == 'success') {
										// 只有微信公众账号 wx_pub 支付成功的结果会在这里返回，其他的 wap
										// 支付结果都是在
										// extra
										// 中对应的URL 跳转。
		...
		.controller(
				'ProgDetailCtrl',
				function($scope, $rootScope, $cookies, $stateParams, $http, $window, $ionicModal, $timeout,
						$ionicLoading) {
					console.log('进度详情...')
					$scope.order = {};
					popLoading($ionicLoading);
					...
					$scope.share = function() {
						...
						shareProduct($scope, $rootScope, $http, $cookies, $timeout, $ionicLoading,
								$scope.order.productVO, $scope.order);
		...
		.controller(
				...
				function($scope, $rootScope, $cookies, $stateParams, $http, $window, $ionicModal, $timeout,
					...
					$scope.share = function() {
						...
						shareProduct($scope, $rootScope, $http, $cookies, $timeout, $ionicLoading,
								$scope.order.productVO, $scope.order, 'Y');
					}
					$scope.orderDetail = function(trackingId) {
						$window.location.href = '#/tab/order/' + trackingId;
					}
		...
		.controller(
				'MineCtrl',
				function($scope, $rootScope, $http, $window, $cookies, $timeout, $ionicLoading, $ionicModal,
						AuthService) {
					console.log('我的打价...');
					modalInit($rootScope, $ionicModal, 'login');
		...
		.controller(
				'MyOrdersCtrl',
				function($scope, $http, $window, $stateParams, $timeout, $ionicLoading) {
					console.log('我的订单...');
					$scope.page = {
						hasMore : false,
						pageNo : 1
					...
					var loadOrders = function() {
						return $http.get('/user/myorders/' + $scope.page.pageNo).success(
								function(data, status, headers, config) {
									$scope.page.hasMore = data.hasNext;
									$scope.page.pageNo = data.currentPage;
									if (null == $scope.myOrders) {
						...
						return $http.get('/user/order/' + $stateParams.trackingId).success(
								function(data, status, headers, config) {
									...
									$scope.checkLogisticUrl = "http://m.kuaidi100.com/index_all.html?type="
											+ data.logisticAgent + "&postid=" + data.logisticTrackingId
		...
		.controller(
				...
				function($scope, $http, $stateParams, $window, $timeout, $ionicLoading) {
					...
					popLoading($ionicLoading);
					$http
							.get('/user/contact/' + $stateParams.contactId)
							.success(
									function(data, status, headers, config) {
										console.log(data);
...
var couponModalInit = function($scope, $ionicModal, CouponService, $timeout, $ionicLoading) {
	...
	$scope.requestCoupon = function(couponId, amt) {
		...
		CouponService.requestCoupon(couponId, amt, function(data) {
			...
			$ionicLoading.hide();
			if (data.succeed) {
				popWarning('领取成功', $timeout, $ionicLoading);
			} else {
				popWarning(data.userMsg, $timeout, $ionicLoading);
			}
			$scope.closeCouponModal();
		}, function(data) {
			console.log(data);
			$ionicLoading.hide();
			popWarning('系统正忙，请稍后重试', $timeout, $ionicLoading);
			$scope.closeCouponModal();
		})
	}
}
